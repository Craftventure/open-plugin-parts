...
class ZeppelinPhysicsController : PhysicsController() {
...
//        val oversteerCurveForSpeed = kart.properties.steer.oversteerCurveForSpeed?.get() ?: LineairPointCurve.empty
        ...
        val zeppelinLifter = properties.zeppelinLifter
        if (zeppelinLifter != null && isControlsEnabled) {
            if (controller.isHandbraking() || kart.isParked()) {
                if (velocity.y < zeppelinLifter.flyVelocityUpSpeedLimit.force)
                    velocity.y += zeppelinLifter.flyVelocityUpSpeed.force
            }
            if (controller.isDismounting()) {
                if (velocity.y > zeppelinLifter.flyVelocityDownSpeedLimit.force)
                    velocity.y -= zeppelinLifter.flyVelocityDownSpeed.force
            }
        }
