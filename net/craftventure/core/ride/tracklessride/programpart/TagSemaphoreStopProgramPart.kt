...
class TagSemaphoreStopProgramPart(
    private val data: Data,
    scene: TracklessRideScene,
) : ProgramPart<Any>(scene) {
    override val type: String = TagSemaphoreStopProgramPart.type
    ...
    override fun execute(
        ...
        data.value.forEach { group.removeTag(TagContext.GLOBAL, it) }
    ...
    data class Data(
        ...
        override fun toPart(scene: TracklessRideScene): ProgramPart<Any> = TagSemaphoreStopProgramPart(this, scene)
    }

    companion object {
        const val type = "tag_semaphore_stop"
    }
}